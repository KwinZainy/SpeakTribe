...
const Menu = () => {
  ...
  return (
       <div>
         ...
         <div >
         {drop&&(
           <div  className=' bg-white w-1/2 flex flex-col gap-12 items-end  h-dvh py-10 fixed top-0 right-0  z-1000 overflow-y-hidden'> 
               <div className=' flex items-center max-mobile:gap-40  max-tablet:gap-90'>
                  <div onClick={(()=>setDrop(!drop))} className=' left-0'>
              {drop? <ImCancelCircle size={35}/>: <IoPersonCircleSharp size={40} />} 
               </div>
               </div>
